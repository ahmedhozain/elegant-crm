...
def completed_interview():
    employee_names = load_list(EMPLOYEES_FILE)  # هذا السطر موجود بالفعل ويضمن ظهور كل الموظفين
    interviewers = ["استاذه داليا", "مستر وليد", "مستر ابراهيم", "محمود", "الاء","محمد ياسر"]
    if request.method == 'POST':
        data = request.form
        conn = get_db_connection()
        conn.execute('''
